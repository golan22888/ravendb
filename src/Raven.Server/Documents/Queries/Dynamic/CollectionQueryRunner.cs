...
namespace Raven.Server.Documents.Queries.Dynamic
{
    public sealed class CollectionQueryRunner : AbstractDatabaseQueryRunner
    {
        ...
        public override async Task<IOperationResult> ExecuteDeleteQuery(IndexQueryServerSide query, QueryOperationOptions options, QueryOperationContext queryContext, Action<IOperationProgress> onProgress, OperationCancelToken token)
        {
            ...
            var result = await runner.ExecuteDelete(query.Metadata.CollectionName, query.Start, query.PageSize, new CollectionOperationOptions
            {
                MaxOpsPerSecond = options.MaxOpsPerSecond
            }, onProgress, token);

            if (options.IndexOptions.WaitForIndexes)
                await WaitForIndexesAfterPatch(query, options, queryContext, token);
        ...
        {
            var runner = new CollectionRunner(Database, queryContext.Documents, query);
            var result = await runner.ExecutePatch(query.Metadata.CollectionName, query.Start, query.PageSize,
                new CollectionOperationOptions { IgnoreMaxStepsForScript = options.IgnoreMaxStepsForScript, MaxOpsPerSecond = options.MaxOpsPerSecond }, patch, patchArgs,
            onProgress, token);
            if (options.IndexOptions.WaitForIndexes) 
                await WaitForIndexesAfterPatch(query,options, queryContext, token);
