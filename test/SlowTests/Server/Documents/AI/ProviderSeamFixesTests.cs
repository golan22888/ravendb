...
namespace SlowTests.Server.Documents.AI
{
    public class ProviderSeamFixesTests(ITestOutputHelper output) : RavenTestBase(output)
    {
        ...
        public async Task EveryOpenAiFamilyProvider_StripsRavenBookkeeping_AndKeepsEveryRealMessageField()
        {
            foreach (var settings in OpenAiFamily())
                await AssertStripsRavenBookkeeping(settings);
        }

        private static async Task AssertStripsRavenBookkeeping(AbstractChatCompletionClientSettings settings)
        {
            using var contextPool = NewContextPool();
            using var client = NewClient(settings);
            ...
            using (var stream = new MemoryStream())
            {
                ...
                {
                    client.Settings.WritePayload(writer, ctx, new AiChatRequest
                    {
                        Messages = [message],
                        UseTools = true,
            ...
            // ours, and never produced by this wire - stripped so a conversation moved off Anthropic stays valid
            Assert.Null(sent[AnthropicChatCompletionClientSettings.RawContentSidecarProperty]);

            // not ours to strip: this wire has always sent them, and widening the list would change what the
            // existing providers receive
            Assert.NotNull(sent[ConversationDocument.SummaryProperty]);
            Assert.NotNull(sent[ChatCompletionClient.Constants.ResponseFields.SubConversationId]);
        ...
        public async Task AnthropicTools_EveryToolIsStrictAndClosed()
        {
            ...
            using (var stream = new MemoryStream())
            {
                ...
                [
                    new AiToolDescriptor("first", "d1", @"{""type"":""object"",""properties"":{""a"":{""type"":""string""}}}"),
        ...
        [RavenFact(RavenTestCategory.Ai)]
        public async Task CanonicalSchemaIsNotMutated()
        {
            const string canonical = @"{""type"":""object"",""properties"":{""city"":{""type"":""string""}}}";
            var descriptor = new AiToolDescriptor("get_weather", "d", canonical);
        ...
        [InlineData(true)]
        [InlineData(false)]
        public async Task ToolsAreOmittedEntirely_WhenTheProviderCannotHonourToolChoiceNone(bool useTools)
        {
            // Ollama ignores tool_choice, so sending "none" would not stop it calling tools - the array has to go.
            var settings = new OllamaChatCompletionClientSettings(new OllamaSettings { Uri = "http://localhost:11434", Model = "m" });
            Assert.False(settings.SupportsToolChoiceNone);

            using var contextPool = NewContextPool();
            using (contextPool.AllocateOperationContext(out JsonOperationContext ctx))
            using (var stream = new MemoryStream())
            {
                var tool = ctx.ReadObject(new DynamicJsonValue
                {
                    ["type"] = "function",
                    ["function"] = new DynamicJsonValue { ["name"] = "get_weather", ["parameters"] = new DynamicJsonValue() }
                }, "tool");

                await using (var writer = new AsyncBlittableJsonTextWriter(ctx, stream))
                {
                    settings.WritePayload(writer, ctx, new AiChatRequest
                    {
                        Messages = [UserMessage(ctx, "hi")],
                        PreparedTools = [tool],
                        UseTools = useTools,
                ...
                var body = JObject.Parse(Encoding.UTF8.GetString(stream.ToArray()));
