...
namespace SlowTests.Server.Documents.AI
{
    public class RetryClassificationTests : RavenTestBase
    {
        ...
        [RavenFact(RavenTestCategory.Ai)]
        public async Task OpenAi_429_WithRetryAfterSeconds_IsRetryable_WithZeroDelay()
        {
            // The header makes it retryable; its value is not read.
            var ex = await Assert.ThrowsAsync<RateLimitException>(() => RunOpenAi(TooManyRequests(h =>
        ...
        [RavenFact(RavenTestCategory.Ai)]
        public async Task OpenAi_429_WithSignal_MalformedResetHeader_ThrowsFormatException()
        {
            await Assert.ThrowsAsync<FormatException>(() => RunOpenAi(TooManyRequests(h =>
            {
        ...
        [RavenFact(RavenTestCategory.Ai)]
        public async Task Anthropic_Permanent400_IsAnUnsuccessfulRequest()
        {
        ...
        [RavenFact(RavenTestCategory.Ai)]
        public async Task Anthropic_Permanent401_IsAnUnsuccessfulRequest()
        {
            var ex = await Assert.ThrowsAsync<UnsuccessfulAiRequestException>(() => RunAnthropic(
        ...
        // ---- a non-429 overflow with a Retry-After: a rate limit on the OpenAI family, as it has always been; -----
        // ---- Anthropic keeps "prompt is too long" non-retryable --------------------------------------------------

        [RavenFact(RavenTestCategory.Ai)]
        public async Task OpenAi_ContextOverflow400_WithRetryAfter_IsARateLimit()
        {
            await Assert.ThrowsAsync<RateLimitException>(() => RunOpenAi(
        ...
        [RavenFact(RavenTestCategory.Ai)]
        public async Task Google_TokenOverflow400_WithRetryAfter_IsARateLimit()
        {
            await Assert.ThrowsAsync<RateLimitException>(() => RunGoogle(
                Error(HttpStatusCode.BadRequest,
        ...
        {
            await Assert.ThrowsAsync<TooManyTokensException>(() => RunAnthropic(
                Error(HttpStatusCode.BadRequest,
